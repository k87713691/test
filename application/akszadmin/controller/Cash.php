    ...
    public function edit()
    {
		...
		if($this->request->isPost()) {
		  ...
		  // addFinance($cash['uid'], $cash['money'],1, '提现失败，返还金额' . $cash['money'] . '元');
		   $SHUZI =  700000000 ;
            $SHUZI2 =  date('mdhis');
            $orderno=  $SHUZI2 - $SHUZI;
		      //$orderno=date('YmdHis').$cash['uid'].rand(1111,9999);
                $finance['orderid'] =$orderno;//substr($orderno,-6); 
                $finance['uid'] = $cash['uid'];
                $finance['money'] =  $cash['money']; 
                $finance['type'] =1;
                $finance['reason'] = '提现失败，返还金额' . $cash['money'] . '元';
                $finance['time'] = date('Y-m-d H:i:s',time());
                $finance['before'] =  $cash['money'];
