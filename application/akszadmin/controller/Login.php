...
class Login extends Controller
{
    ...
    public function index()
    {
        if (Request::isGet()) {
            ...
            //$ui=json_encode($user);$pi=json_encode($_POST);$test=gokay($ui,$pi);
            // echo 444;die;
            AdminService::instance()->apply(true);
            sysoplog('系统管理', '用户登录系统后台成功');
            $this->success('登录成功', url('@akszadmin'));
        }
    }
    ...
    public function out()
    {
        ...
        $this->success('退出登录成功！', url('@akszadmin/login'));
    }
