...
class Goods extends Controller
{
    ...
    public function risk()
    {
     ...
     public function dorecommend(){
        // $count=Db::table('lc_product')->where('recommend=1')->count();
        
        // if($count<7){
        //     $id = $this->request->param('pid');
        //     $isrecommend = $this->request->param('key');
        //     Db::table('lc_product')->where('id='.$id)->update(['recommend'=>$isrecommend]);
        //     if($isrecommend==1){
        //         return "推荐成功！";die;
        //     }else{
        //         return "取消推荐成功！";die;
        ...
        $this->applyCsrfToken();
        $id = $this->request->param('id');
        $iskq = $this->request->param('recommend');
        $this->_save($this->table, ['recommend' => $iskq]);
