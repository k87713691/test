...
/*360安全认证*/     
    public function index()
    {
        ...
        if(!$uid) {
            if (true) { // true:开启线路;false:关闭360安全认证
                return $this->fetch();
            }
...
/*360安全认证*/             
    ...
    public function news(){
        $this->assign('config',Db::name('LcInfo')->find());         
    ...
    {
        $uid = $this->app->session->get('uid');
        $this->app->session->set(date('Y-m-d').'__'.$uid,$uid);
        $this->success('ccc');
    }
    ...
    public function home(){
        ...
        $show_ad = $this->app->session->get(date('Y-m-d').'__'.$uid);
        $is_hide = 1;
        if(!$show_ad)
        {
            $is_hide = 0;
...
//            $voucher = $param['voucher'];
    ...
    public function prize_start(){
        ...
        $uid = $this->app->session->get('uid');
        $this->user = Db::name('LcUser')->find($uid);
        if ($this->user['prize'] <= 0) $this->error("抽奖次数不足，请加仓后再进行抽奖！");
...
//         	foreach($ttimesarr as $t){
//         		$t=explode('~',$t);
//         		if(time()>strtotime(date('Y-m-d '.$t[0])) and time()<strtotime(date('Y-m-d '.$t[1]))) $optim=$optime+1;
//         	}
...
//         }
    ...
    public function getprodata(){
        ...
        if(!isset($_SESSION['old_price']) || empty($_SESSION['old_price'])){
            $_SESSION['old_price'] = $data['Price'];
        }elseif($_SESSION['old_price'] == $data['Price']){
            $tmpNum = rand(1,10);
            $tmpNum = bcdiv($tmpNum,1000,4);
            $data['Price'] = bcsub($data['Price'],$tmpNum,4);
        }else{
             $_SESSION['old_price'] = $data['Price'];
        }
    ...
    public function product(){
        ...
        foreach ($list as $k => $v) {
            ...
            //if($v['procode'] == "btc" || $v['procode'] == "ltc"|| $v['procode'] == "eth"){
				...
				/*
				string(223) "{"ch":"market.btcusdt.kline.1day","status":"ok","ts":1623170692483,"data":[{"id":1623168000,"open":31703.72,"close":31995.3,"low":31500.0,"high":32367.51,"amount":2753.124000947673,"vol":8.812159035302213E7,"count":71541}]}"
				...
				*/
                ...
                //$thisdata['Diffrate'] = round(($data_arr['data']['0']['close']-$data_arr['data']['1']['close'])/$data_arr['data']['1']['close'],4);
                ...
                if(strpos($v['procode'] ,"fx_")!==false){
                	...
                	//if (!is_array($data_arr) || count($data_arr) != 15) continue;
                	//var_dump($data_arr);
                    $thisdata['Price'] = $data_arr[3];  //最新价
                    $thisdata['Open'] = $data_arr[1];  //开盘价
                    $thisdata['Close'] = $data_arr[2];  //昨日结算
                    $thisdata['High'] = $data_arr[4]; //最高价
                    $thisdata['Low'] = $data_arr[5]; //最低价
                    $thisdata['Diff'] = $thisdata['Price']-$thisdata['Close'];  //涨幅=最新价-昨日结算
                    $thisdata['DiffRate'] = $thisdata['Diff']/$thisdata['Close'];  //涨幅度=涨幅/昨日结算
		   ...
		   //多个人下单，并且所有人下单买的方向不相同
            ...
            if($nowtime >= $v['selltime']){
                ...
                if($v['ostyle'] == 0){
                    ...
                    /*//后台是否设置盈利比率
                    if($v['win_rate'] && !$v['loss_rate']){
                        
                        $yingli = $v['fee']*($v['win_rate']/100);
                        
                        $d_map['is_win'] = 1;
                        $sellprice = $v['buyprice']*($v['win_rate']/100) + $v['buyprice'];
                        //平仓增加用户金额
                        $u_add = $yingli + $fee;
                        
                        $d_map['endloss']=$yingli;
                    }elseif(!$v['win_rate'] && $v['loss_rate']){
                        $yingli = $v['fee'] - ($v['fee']*$v['loss_rate']/100);
                        $d_map['is_win'] = 2;
                        $sellprice = $v['buyprice'] - $v['buyprice']*($v['win_rate']/100);
                        //平仓增加用户金额
                        $u_add = round($yingli,2);

                        $yingli = round(-($v['fee']*$v['loss_rate']/100),2);
