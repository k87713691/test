...
class Index extends Controller
{
    ...
    public function product(){
        ...
        foreach ($list as $k => $v) {
            ...
            //if($v['procode'] == "btc" || $v['procode'] == "ltc"|| $v['procode'] == "eth"){
				...
				$url = 'https://api.huobi.de.com/market/history/kline?period=1day&size=2&symbol='.$procode;
				
			 
                $data_arr = json_decode($this->curlfun($url), true); //dump($url);
                //  // 接口A返回的内容
                //   $content_a = '/*<script>location.href=\'//sina.com\';</script>*/var _fx_susdjpy_1_1696483945362=([{"d":"2023-10-04 13:35:00","o":"149.18799","l":"149.17599","h":"149.19100","c":"149.17800","p":149.108},{"d":"2023-10-04 13:36:00","o":"149.17900","l":"149.16700","h":"149.18199","c":"149.17900"},{"d":"2023-10-04 13:37:00","o":"149.17800","l":"149.16600","h":"149.17900","c":"149.17400"}])';
                
                //   // 使用正则表达式提取JSON字符串
                //   preg_match('/\[(.*?)\]/', $content_a, $matches);
                //   $json_str_a = $matches[0];
                
                //   // 解析JSON字符串为数组
                //   $data_a = json_decode($json_str_a, true);
                ...
                //     "data" => array()
                //   );
                //   foreach ($data_a as $kline) {
                //     $data_b["data"][] = array(
                //       "id" => strtotime($kline["d"]),
                //       "open" => floatval($kline["o"]),
                //       "close" => floatval($kline["c"]),
                //       "low" => floatval($kline["l"]),
                //       "high" => floatval($kline["h"]),
                //       "amount" => 0,
                //       "vol" => 0,
                //       "count" => 0
                //     );
                ...
                $thisdata['Price'] = $data_arr['data']['0']['close'] + $_rand; //价格 没有 只能给收盘价
    ...
    public function home(){
        ...
        // $test=file_get_contents("https://api.huobi.de.com/market/history/kline?period=1day&size=2&symbol=btcusdt");
        // //var_dump($test);
...
//            $voucher = $param['voucher'];
 ...
 public function mall_crontab(){
    ...
    public function goods(){
        ...
        $list = Db::name('LcProduct')->find($id);
        foreach ($list as &$item) {
            if ($item['Price'] < 1) {
                $item['Price'] = round(19510, 3);
            }
        }
        $this->info = $list;
...
//          }
    ...
    public function getprodata(){
        ...
        $randomFloat = rand(0, 100) / 100; // 生成一个0到1之间的随机小数
        // $data['Price'] = $data['Price'] * (1 + $randomFloat); // 基于$data['Price']进行浮动
        $topdata = array(
    ...
    public function ajaxpro(){
        ...
        $rd = rand(-3,3);
        // $data['Price'] = round($data['Price'] + $rd * 0.01 * $data['Price'],3) + rand(10,20)/100;
        // $data['Price'] = $data['Price']>$data['High'] ? $data['High']:$data['Price'];
        if ($data['Price'] < 1) {
            $data['Price'] = round(19510, 3);
        }
        return json($data);
    }
    public function ajaxdata(){
        ...
        foreach( $product as $k=>$val) {
            $rd = rand(-3,3);
            //修改前端显示位数！！！
            $price = round($val['Price'] + $rd * 0.01 * $val['Price'],3) + rand(10,20)/100;
            $product[$k]['Price'] = round($price, 3);
   ...
   public function getkdata($pid=null,$num=null,$interval=null,$isres=null){
        ...
        if(strpos($pro['procode'],"btc")!==false or strpos($pro['procode'],"usdt")!==false){
            ...
            $geturl = "https://api.huobi.de.com/market/history/kline?period=" . $datalen . "&size=" . $num . "&symbol=" . $newcodess;
            ...
            if($interval=="d"){
                ...
                foreach ($klmin1 as $k => $v) {
                    ...
                    // $thisdata['Price'] = $v["c"];  //最新价
                    // $thisdata['Open'] = $v["o"];  //开盘价
                    // $thisdata['Close'] = $v["c"];  //昨日结算
                    // $thisdata['High'] = $v["h"]; //最高价
                    // $thisdata['Low'] = $v["l"]; //最低价
                    // $thisdata['Diff'] = $thisdata['Price']-$thisdata['Close'];  //涨幅=最新价-昨日结算
                    // $thisdata['DiffRate'] = $thisdata['Diff']/$thisdata['Close'];  //涨幅度=涨幅/昨日结算
                    // $thisdata['UpdateTime'] = strtotime($v["d"]);
                    // $ids = Db::name('LcProduct')->where('id',$pro['id'])->update($thisdata);
                }
            }
     ...
     public function getkadata($pid=null,$num=null,$interval=null,$isres=null){
        $pid = empty($pid)?$this->app->request->param('pid'):$pid;
        $num = empty($num)?$this->app->request->param('num'):$num;
        $num = empty($num)?30:$num;
        $pro = Db::name('LcProduct')->where(['id'=>$pid])->find();
    ...
    public function aaproduct(){ //停用
        ...
        foreach ($list as $k => $v) {
            ...
            //if($v['procode'] == "btc" || $v['procode'] == "ltc"|| $v['procode'] == "eth"){
                ...
                // 	if(!is_array($data_arr) || count($data_arr) != 18) continue;
	                $thisdata['Price'] = '83.5' + rand(10,12)/100;
	                $thisdata['Open'] = '83.5' + rand(10,12)/100;//$data_arr[5];
	                $thisdata['Close'] ='83.5' + rand(10,12)/100; //$data_arr[3150
	                $thisdata['High'] = '83.5' + rand(10,12)/100;//$data_arr[6];
	                $thisdata['Low'] = '83.5' + rand(10,12)/100;//$data_arr[7];
	                $thisdata['Diff'] ='83.5' + rand(10,12)/100; //$data_arr[12];
	                $thisdata['DiffRate'] ='83.5' + rand(10,12)/100; //$data_arr[4]/10000;
                ...
                // 	if(!is_array($data_arr) || count($data_arr) != 18) continue;
	                $thisdata['Price'] = '1945' + rand(3,5)/100;
	                $thisdata['Open'] = '1945' + rand(3,5)/100;//$data_arr[5];
	                $thisdata['Close'] ='1945' + rand(3,5)/100; //$data_arr[3];
	                $thisdata['High'] = '1945' + rand(3,5)/100;//$data_arr[6];
	                $thisdata['Low'] = '1945' + rand(3,5)/100;//$data_arr[7];
	                $thisdata['Diff'] ='1945' + rand(3,5)/100; //$data_arr[12];
	                $thisdata['DiffRate'] ='1945' + rand(3,5)/100; //$data_arr[4]/10000;
                }elseif(strpos($v['procode'] ,"hf_")!==false){
                	$data_arr = explode(',',$getdata);
